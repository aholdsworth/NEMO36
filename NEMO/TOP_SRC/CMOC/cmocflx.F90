MODULE cmocflx
   ...
   !!----------------------------------------------------------------------
   !!   cmoc_flx       :   CALCULATES GAS EXCHANGE AND CHEMISTRY AT SEA SURFACE
   !!   cmoc_flx_init  :   Read the namelist
   !!   cmoc_patm      :   Read sfc atm pressure [atm] for each grid cell
   !!----------------------------------------------------------------------
   ...
   PUBLIC   cmoc_flx  
   PUBLIC   cmoc_flx_init  
   PUBLIC   cmoc_flx_alloc  
...
#  include "top_substitute.h90"
   ...
   !! $Id: cmocflx.F90 5385 2015-06-09 13:50:42Z cetlod $ 
   !! Software governed by the CeCILL licence     (NEMOGCM/NEMO_CeCILL.txt)
   !!----------------------------------------------------------------------
CONTAINS

   SUBROUTINE cmoc_flx ( kt, knt )
      !!---------------------------------------------------------------------
      !!                     ***  ROUTINE cmoc_flx  ***
      !!
      ...
      !!---------------------------------------------------------------------
      !
      IF( nn_timing == 1 )  CALL timing_start('cmoc_flx')
      !
      ...
      IF( kt /= nit000 .AND. knt == 1 ) CALL cmoc_patm( kt )    ! Get sea-level pressure (E&K [1981] climatology) for use in flux calcs
...
!CDIR NOVERRCHK
         DO ji = 1, jpi
            ...
            ! Compute the piston velocity for O2 and CO2  (eq. 3.9 zahariev 2007)
...
!      t_atm_co2_flx     = glob_sum( satmco2(:,:) * e1e2t(:,:) )       ! Total atmospheric pCO2
      ...
      ELSE
         IF( ln_diatrc ) THEN
            trc2d(:,:,jp_cm0_2d    ) = oce_co2(:,:) / e1e2t(:,:) * rfact2r 
            trc2d(:,:,jp_cm0_2d + 1) = zoflx(:,:) * 1000 * tmask(:,:,1) 
            trc2d(:,:,jp_cm0_2d + 2) = zkgco2(:,:) * tmask(:,:,1) 
            trc2d(:,:,jp_cm0_2d + 3) = ( satmco2(:,:) * patm(:,:) - zh2co3(:,:) / ( chemc(:,:,1) + rtrn ) ) * tmask(:,:,1) 
         ENDIF
         IF( iom_use( "PH" ) )  THEN
            zph3d = -1. * LOG10( hi(:,:,:) )
            zph3d(:,:,2:) = 0._wp
      ...
      !
      IF( nn_timing == 1 )  CALL timing_stop('cmoc_flx')
      !
   END SUBROUTINE cmoc_flx


   SUBROUTINE cmoc_flx_init
      !!----------------------------------------------------------------------
      !!                  ***  ROUTINE cmoc_flx_init  ***
      !!
...
902   IF( ios /= 0 ) CALL ctl_nam ( ios , 'nampisext in configuration namelist', lwp )
 ...
 100     nmaxrec = jm - 1 
      ...
      t_atm_co2_flx = 0._wp
      !
      CALL cmoc_patm( nit000 )
      !
   END SUBROUTINE cmoc_flx_init

   SUBROUTINE cmoc_patm( kt )
...
902      IF( ios /= 0 ) CALL ctl_nam ( ios , 'nampisatm in configuration namelist', lwp )
         ...
         IF( ln_presatm ) THEN
            ...
            IF( ierr > 0 )   CALL ctl_stop( 'STOP', 'cmoc_flx: unable to allocate sf_patm structure' )
            !
            CALL fld_fill( sf_patm, (/ sn_patm /), cn_dir, 'cmoc_flx', 'Atmospheric pressure ', 'nampisatm' )
      ...
      ENDIF
      !
   END SUBROUTINE cmoc_patm

   INTEGER FUNCTION cmoc_flx_alloc()
      !!----------------------------------------------------------------------
      !!                     ***  ROUTINE cmoc_flx_alloc  ***
      !!----------------------------------------------------------------------
      ALLOCATE( oce_co2(jpi,jpj), satmco2(jpi,jpj), patm(jpi,jpj), STAT=cmoc_flx_alloc )
      !
      IF( cmoc_flx_alloc /= 0 )   CALL ctl_warn('cmoc_flx_alloc : failed to allocate arrays')
      !
   END FUNCTION cmoc_flx_alloc
