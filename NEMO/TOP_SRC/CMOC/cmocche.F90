MODULE cmocche
   ...
   !!----------------------------------------------------------------------
   !!   cmoc_che      :  Sea water chemistry computed following OCMIP protocol
   !!----------------------------------------------------------------------
   ...
   REAL(wp) ::   ox2    =  3.99063      !      Original CMOC code noted this was a solubility, but 
...
#include "top_substitute.h90"
   ...
   !! $Id: cmocche.F90 5215 2015-04-15 16:11:56Z nicolasmartin $ 
...
CONTAINS
   ...
   SUBROUTINE cmoc_che
      ...
      !!---------------------------------------------------------------------
      !
      IF( nn_timing == 1 )  CALL timing_start('cmoc_che')
      !
...
!CDIR NOVERRCHK
      ...
      END DO
      !
      IF( nn_timing == 1 )  CALL timing_stop('cmoc_che')
      !
   END SUBROUTINE cmoc_che


   INTEGER FUNCTION cmoc_che_alloc()
      !!----------------------------------------------------------------------
      !!                     ***  ROUTINE cmoc_che_alloc  ***
      !!----------------------------------------------------------------------
      ALLOCATE( chemc(jpi,jpj,2), STAT=cmoc_che_alloc )
      !
      IF( cmoc_che_alloc /= 0 )   CALL ctl_warn('cmoc_che_alloc : failed to allocate arrays.')
      !
   END FUNCTION cmoc_che_alloc
