MODULE cansbc
   !!======================================================================
   !!                         ***  MODULE p4sbc  ***
   !! TOP :   CanOE surface boundary conditions of external inputs of nutrients
   !!======================================================================
   ...
   !!----------------------------------------------------------------------
   !!   can_sbc        :  Read and interpolate time-varying nutrients fluxes
   !!   can_sbc_init   :  Initialization of can_sbc
   !!----------------------------------------------------------------------
   ...
   REAL(wp), PUBLIC  :: sedfeinput  = 1000._wp   !: Coastal release of Iron
   REAL(wp) , PUBLIC:: dustsolub   = 0.014_wp   !: Solubility of the dust
   REAL(wp), PUBLIC :: wdust       = 2.0_wp     !: Sinking speed of the dust 
   REAL(wp), PUBLIC :: nitrfix     = 2.25E-2_wp !: Nitrogen fixation rate   
   REAL(wp), PUBLIC :: diazolight  = 50._wp     !: Nitrogen fixation sensitivty to light 
   REAL(wp), PUBLIC :: concfediaz  = 100._wp    !: Fe half-saturation Cste for diazotrophs 
   REAL(wp), PUBLIC :: kni         = 0.1_wp     !: half-saturation for NO3 inhibition of diazotrophy
   ...
   REAL(wp), PUBLIC ::  rivalkinput, rivdicinput, nitdepinput
   REAL(wp), PUBLIC :: rivdininput, rivdipinput
...
!   'key_traldf_c3d' :                 aht: 3D coefficient
   ...
   !!----------------------------------------------------------------------
   !! NEMO/TOP 3.3 , NEMO Consortium (2010)
   !! $Id: cansbc.F90 5507 2015-06-29 15:19:49Z aumont $ 
   !! Software governed by the CeCILL licence (NEMOGCM/NEMO_CeCILL.txt)
   !!----------------------------------------------------------------------

CONTAINS

   SUBROUTINE can_sbc( kt )
      !!----------------------------------------------------------------------
      !!                  ***  routine can_sbc  ***
      !!
      ...
      !!---------------------------------------------------------------------
      !
      IF( nn_timing == 1 )  CALL timing_start('can_sbc')
     ...
     ! Compute N deposition at nit000 or only if there is more than 1 time record in N deposition file
      IF( ln_ndepo ) THEN
         IF( kt == nit000 .OR. ( kt /= nit000 .AND. ntimes_ndep > 1 ) ) THEN
            ...
            DO jj = 1, jpj
               DO ji = 1, jpi
                  nitdep(ji,jj) = sf_ndepo(1)%fnow(ji,jj,1) / ( 14E6 * ryyss * e3t_n(ji,jj,1) + rtrn )
               END DO
            END DO
         ENDIF
      ENDIF
      !
     !
      IF( nn_timing == 1 )  CALL timing_stop('can_sbc')
      !
   END SUBROUTINE can_sbc

   SUBROUTINE can_sbc_init

      !!----------------------------------------------------------------------
      !!                  ***  routine can_sbc_init  ***
      !!
      ...
      TYPE(FLD_N) ::   sn_dust,  sn_ndepo, sn_ironsed   ! informations about the fields to be read
      ...
      !
      NAMELIST/nampissbc/cn_dir, sn_dust,  sn_riverdic, sn_riverdoc, sn_riverdin, sn_riverdon,     &
        &                sn_riverdip, sn_riverdop, cn_dir, sn_ndepo, sn_ironsed, &
        &                ln_dust,  ln_river, ln_ndepo, ln_ironsed,      &
        &                sedfeinput, dustsolub, wdust,  nitrfix, diazolight, concfediaz
      !!----------------------------------------------------------------------
      !
      IF( nn_timing == 1 )  CALL timing_start('can_sbc_init')
      !
...
902   IF( ios /= 0 ) CALL ctl_nam ( ios , 'nampissbc in configuration namelist', lwp )
      ...
      IF( ln_dust ) THEN 
         ...
         IF( ierr > 0 )   CALL ctl_stop( 'STOP', 'can_sed_init: unable to allocate sf_dust structure' )
         !
         CALL fld_fill( sf_dust, (/ sn_dust /), cn_dir, 'can_sed_init', 'Iron from sediments', 'nampissed' )
...
!         !
         ...
         !, rivdin(jpi,jpj), rivdip(jpi,jpj), rivdsi(jpi,jpj) ) 
!         !
         ALLOCATE( sf_river(jpriv), rivinput(jpriv), STAT=ierr1 )           !* allocate and fill sf_river (forcing structure) with sn_river_
!         rivinput(:) = 0.0
!
         IF( ierr1 > 0 )   CALL ctl_stop( 'STOP', 'can_sed_init: unable to allocate sf_irver structure' )
!         !
         CALL fld_fill( sf_river, slf_river, cn_dir, 'can_sed_init', 'Input from river ', 'nampissed' )
         DO ifpr = 1, jpriv
...
!            ! N/P and Si releases due to coastal rivers
!            ! -----------------------------------------
            rivdicinput = (rivinput(jr_dic) + rivinput(jr_doc) ) * 1E9 / 31.6_wp
...
!            rivdsiinput = rivinput(jr_dsi) * 1E3 / 28.1_wp
            rivalkinput = rivinput(jr_dic) * 1E9 / 12._wp
!            !
         ENDIF
      ELSE
         rivdicinput = 0._wp
...
!         rivdsiinput = 0._wp
      ...
      IF( ln_ndepo ) THEN
         ...
         IF( ierr3 > 0 )   CALL ctl_stop( 'STOP', 'can_sed_init: unable to allocate sf_ndepo structure' )
         !
         CALL fld_fill( sf_ndepo, (/ sn_ndepo /), cn_dir, 'can_sed_init', 'Nutrient atmospheric depositon ', 'nampissed' )
      ...
      IF(lwp) THEN 
         ...
         WRITE(numout,*) '    DIC Supply : ', rivdicinput*2.631*1E3*12./1E12,'TgC/yr'
         !WRITE(numout,*) 
      ...
      ENDIF
      !
      IF( nn_timing == 1 )  CALL timing_stop('can_sbc_init')
      !
   END SUBROUTINE can_sbc_init
