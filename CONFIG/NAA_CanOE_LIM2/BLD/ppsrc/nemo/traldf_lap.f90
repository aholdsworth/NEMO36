         ...
         DO jk = 1, jpkm1                                            ! slab loop
            ...
            DO jj = 1, jpjm1
               DO ji = 1, jpi   ! vector opt.
                  zabe1 = rldf * ahtu(ji,jj,jk) * r_fact_lap(ji,jj,jk) * umask(ji,jj,jk) * re2u_e1u(ji,jj) * e3u_n(ji,jj,jk)
                  zabe2 = rldf * ahtv(ji,jj,jk) * vmask(ji,jj,jk) * re1v_e2v(ji,jj) * e3v_n(ji,jj,jk)
            ...
            IF( ln_zps ) THEN      ! set gradient at partial step level for the last ocean cell
               DO jj = 1, jpjm1
                  DO ji = 1, jpi   ! vector opt.
                     ...
                     ikv = mbkv(ji,jj)
                     IF( iku == jk ) THEN
                        zabe1 = rldf * ahtu(ji,jj,iku) * r_fact_lap(ji,jj,iku) * umask(ji,jj,iku) * re2u_e1u(ji,jj) * e3u_n(ji,jj,iku)
                        ztu(ji,jj,jk) = zabe1 * pgu(ji,jj,jn)
                     ENDIF
                     IF( ikv == jk ) THEN
                        zabe2 = rldf * ahtv(ji,jj,ikv) * vmask(ji,jj,ikv) * re1v_e2v(ji,jj) * e3v_n(ji,jj,ikv)
