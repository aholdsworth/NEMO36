...
!     have to be replaced in the code by xxx_n
   ...
   !!----------------------------------------------------------------------
   !! NEMO/OPA 3.3 , NEMO-Consortium (2010) 
   !! $Id: tranxt.F90 8305 2017-07-10 09:33:34Z davestorkey $
   !! Software governed by the CeCILL licence     (NEMOGCM/NEMO_CeCILL.txt)
   !!----------------------------------------------------------------------
CONTAINS
   ...
   SUBROUTINE tra_nxt( kt )
      ...
      IF( l_trdtra )   THEN                    
         ...
         IF( lk_vvl ) THEN
            DO jk = 1, jpkm1
               zfact = 1.0 / rdttra(jk)
               ztrdt(:,:,jk) = ( tsa(:,:,jk,jp_tem)*e3t_0(:,:,jk) / e3t_0(:,:,jk) - tsn(:,:,jk,jp_tem)) * zfact
               ztrds(:,:,jk) = ( tsa(:,:,jk,jp_sal)*e3t_0(:,:,jk) / e3t_0(:,:,jk) - tsn(:,:,jk,jp_sal)) * zfact
            END DO
         ELSE
            DO jk = 1, jpkm1
               zfact = 1.0 / rdttra(jk)
