...
CONTAINS
   ...
   SUBROUTINE tra_zdf( kt )
      ...
      IF( l_trdtra )   THEN                      ! save the vertical diffusive trends for further diagnostics
         ...
         IF( lk_vvl ) THEN
            DO jk = 1, jpkm1
               ztrdt(:,:,jk) = ( ( tsa(:,:,jk,jp_tem)*e3t_0(:,:,jk) - tsb(:,:,jk,jp_tem)*e3t_0(:,:,jk) ) &
                    & / (e3t_0(:,:,jk)*r2dtra(jk)) ) - ztrdt(:,:,jk)
               ztrds(:,:,jk) = ( ( tsa(:,:,jk,jp_sal)*e3t_0(:,:,jk) - tsb(:,:,jk,jp_sal)*e3t_0(:,:,jk) ) &
                    & / (e3t_0(:,:,jk)*r2dtra(jk)) ) - ztrds(:,:,jk)
            END DO
         ELSE
            DO jk = 1, jpkm1
