      ...
      DO jj = 2, jpjm1        !==  second recurrence:    SOLk = RHSk - Lk / Dk-1  Lk-1  ==
         DO ji = 2, jpim1   ! vector opt.
            va(ji,jj,1) = vb(ji,jj,1) &
               ...
               &                                      / ( e3v_0(ji,jj,1) * rau0     ) * vmask(ji,jj,1) )
         END DO
      END DO
      DO jk = 2, jpkm1
         DO jj = 2, jpjm1
      ...
      IF( iom_use( 'dispkevfo' ) ) THEN   ! ocean kinetic energy dissipation per unit area
         ...
         DO jk = 1, jpkm1
            DO jj = 2, jpjm1
               DO ji = 2, jpim1
                  z2d(ji,jj) = z2d(ji,jj)  +  (                                                                                  &
                     &   avmu(ji  ,jj,jk) * ( ua(ji  ,jj,jk-1) - ua(ji  ,jj,jk) )**2 / e3uw_0(ji  ,jj,jk) * wumask(ji  ,jj,jk)   &
                     & + avmu(ji-1,jj,jk) * ( ua(ji-1,jj,jk-1) - ua(ji-1,jj,jk) )**2 / e3uw_0(ji-1,jj,jk) * wumask(ji-1,jj,jk)   &
                     & + avmv(ji,jj  ,jk) * ( va(ji,jj  ,jk-1) - va(ji,jj  ,jk) )**2 / e3vw_0(ji,jj  ,jk) * wvmask(ji,jj  ,jk)   &
                     & + avmv(ji,jj-1,jk) * ( va(ji,jj-1,jk-1) - va(ji,jj-1,jk) )**2 / e3vw_0(ji,jj-1,jk) * wvmask(ji,jj-1,jk)   &
                     &                        )
               END DO
            END DO
         END DO
