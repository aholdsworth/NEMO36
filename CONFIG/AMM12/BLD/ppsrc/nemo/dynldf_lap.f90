      ...
      IF( iom_use('dispkexyfo') ) THEN   ! ocean Kinetic Energy dissipation per unit area
         ...
         DO jk = 1, jpkm1
            DO jj = 2, jpjm1
               DO ji = 2, jpim1
                  z2d(ji,jj) = z2d(ji,jj)          -   (                                                         &
                     &   hdivb(ji,jj,jk)**2 * ahm1(ji,jj,jk) * e3t_0(ji,jj,jk)                               &
                     & + 0.25_wp * (                                                                             &
                     &   rotb (ji  ,jj  ,jk)**2 * ahm2(ji  ,jj  ,jk) * e12f(ji  ,jj  ) * e3f_0(ji  ,jj  ,jk)   &
                     & + rotb (ji-1,jj  ,jk)**2 * ahm2(ji-1,jj  ,jk) * e12f(ji-1,jj  ) * e3f_0(ji-1,jj  ,jk)   &
                     & + rotb (ji  ,jj-1,jk)**2 * ahm2(ji  ,jj-1,jk) * e12f(ji  ,jj-1) * e3f_0(ji  ,jj-1,jk)   &
                     & + rotb (ji-1,jj-1,jk)**2 * ahm2(ji-1,jj-1,jk) * e12f(ji-1,jj-1) * e3f_0(ji-1,jj-1,jk)   &
