      ...
      IF( iom_use('dispkexyfo') ) THEN   ! ocean kinetic energy dissipation per unit area
         ...
         DO jk = 1, jpkm1
            DO jj = 2, jpjm1
               DO ji = 2, jpim1
                  z2d(ji,jj) = z2d(ji,jj)                                                                  &
                     &  +  (  ahm3(ji,jj,jk)*zlu(ji,jj,jk)**2 + ahm3(ji-1,jj,jk)*zlu(ji-1,jj,jk)**2    &
                     &      + ahm4(ji,jj,jk)*zlv(ji,jj,jk)**2 + ahm4(ji,jj-1,jk)*zlv(ji,jj-1,jk)**2  ) * tmask(ji,jj,jk)
               END DO
            END DO
         END DO
         zzz = 0.5_wp * rau0
