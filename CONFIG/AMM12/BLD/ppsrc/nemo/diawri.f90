   ...
   SUBROUTINE dia_wri( kt )
      ...
      IF ( iom_use("eken") ) THEN
         ...
         DO jk = 1, jpkm1
            DO jj = 2, jpjm1
               DO ji = 2, jpim1   ! vector opt.
                  zztmp   =  1 / (e1e2t(ji,jj) * e3t_0(ji,jj,jk))
                  zztmpx  = 0.5 * (  un(ji-1,jj,jk) * un(ji-1,jj,jk) * e1u(ji-1,jj) * e2u(ji-1,jj) * e3u_0(ji-1,jj,jk)    &
                     &             + un(ji  ,jj,jk) * un(ji  ,jj,jk) * e1u(ji,  jj) * e2u(ji  ,jj) * e3u_0(ji  ,jj,jk) )  &
                     &          *  zztmp 
                  !
                  zztmpy  = 0.5 * (  vn(ji,jj-1,jk) * vn(ji,jj-1,jk) * e1v(ji,jj-1) * e2v(ji,jj-1) * e3v_0(ji,jj-1,jk)    &
                     &             + vn(ji,jj  ,jk) * vn(ji,jj  ,jk) * e1v(ji,jj  ) * e2v(ji,jj  ) * e3v_0(ji,jj  ,jk) )  &
                     &          *  zztmp 
                  !
      ...
      IF( iom_use("tosmint") ) THEN
         ...
         DO jk = 1, jpkm1
            DO jj = 2, jpjm1
               DO ji = 2, jpim1   ! vector opt.
                  z2d(ji,jj) = z2d(ji,jj) + rau0 * e3t_0(ji,jj,jk) *  tsn(ji,jj,jk,jp_tem)
               END DO
            END DO
         END DO
         CALL lbc_lnk( z2d, 'T', -1. )
      ...
      IF( iom_use("somint") ) THEN
         ...
         DO jk = 1, jpkm1
            DO jj = 2, jpjm1
               DO ji = 2, jpim1   ! vector opt.
                  z2d(ji,jj) = z2d(ji,jj) + rau0 * e3t_0(ji,jj,jk) * tsn(ji,jj,jk,jp_sal)
               END DO
            END DO
         END DO
         CALL lbc_lnk( z2d, 'T', -1. )
