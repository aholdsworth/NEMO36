         ...
         DO jk = 1, jpkm1                                            ! slab loop
            ...
            DO jj = 1, jpjm1
               DO ji = 1, jpim1   ! vector opt.
                  zabe1 = rldf * ahtu(ji,jj) * r_fact_lap(ji,jj,jk) * umask(ji,jj,jk) * re2u_e1u(ji,jj) * e3u_0(ji,jj,jk)
            ...
            IF( ln_zps ) THEN      ! set gradient at partial step level for the last ocean cell
               DO jj = 1, jpjm1
                  DO ji = 1, jpim1   ! vector opt.
                     ...
                     ikv = mbkv(ji,jj)
                     IF( iku == jk ) THEN
                        zabe1 = rldf * ahtu(ji,jj) * r_fact_lap(ji,jj,iku) * umask(ji,jj,iku) * re2u_e1u(ji,jj) * e3u_0(ji,jj,iku)
